...
?>
	...
	<div class="lean-posts row <?php if($instance['posts_thumb']) { echo 'thumbs-enabled'; } ?>">
		<?php while ($posts->have_posts()): $posts->the_post(); ?>
		<div class="col-4">
		<div class="card card-inverse card-posts card-title-overlay" style="border: 0px;">
			...
			<?php if($instance['posts_category']) {
				...
				foreach((get_the_category()) as $category) {
					echo  '<a href="'. $category->category_link.'"><span class="badge badge-info">'.$category->cat_name.'</span></a>&nbsp;';
				}
				echo "</p>";
			} ?>

			<div class="card-img-footer">
				<h3 class="card-title"><a href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title(); ?>"><?php the_title(); ?></a></h3>
				<?php if($instance['posts_date']) { ?><p class="card-text"><small class="text-muted"><?php the_time('j M, Y'); ?></small></p><?php } ?>
			</div>
